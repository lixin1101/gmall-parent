package com.lx.exportsale.app

import com.lx.constants.GmallConstants
import com.lx.exportsale.bean.SaleDetailDaycount
import com.lx.utils.MyEsUtil
    ...
    saleDetailDaycountRDD.foreachPartition { saleItr =>
      ...
      for (saleDetail <- saleItr) {
        ...
        if (listBuffer.size == 100) {
          MyEsUtil.indexBulk(GmallConstants.ES_INDEX_SALE_DETAIL, listBuffer.toList)
          listBuffer.clear()
        }
      }
